...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], opt=None,mlp_init=False):
    ...
    def forward1(self, feats, ext_range, num_patches=64, patch_ids=None):
        return_ids = []
        return_feats = []
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
    ...
    def create_mlp1(self, feats, patch_size ):
        for mlp_id, feat in enumerate(feats):
            input_nc = feat.shape[1]*patch_size*patch_size
            mlp = nn.Sequential(*[nn.Linear(input_nc, self.nc), nn.ReLU(), nn.Linear(self.nc, self.nc)])
            if len(self.gpu_ids) > 0:
                mlp.cuda()
            setattr(self, 'mlp_%d' % mlp_id, mlp)
    ...
    def forward2(self, feats, num_patches=64, patch_size=16, patch_ids=None, stride=8):
        return_ids = []
        return_feats = []

        if self.use_mlp and not self.mlp_init:
            self.create_mlp1(feats, patch_size)
    
        for feat_id, feat in enumerate(feats):

            B, C, H, W = feat.shape

            patches = feat.unfold(2, patch_size, stride).unfold(3, patch_size, stride)  # [B, C, num_patches_h, num_patches_w, patch_size, patch_size]
            patches = patches.contiguous().view(B, C, -1, patch_size, patch_size)  # [B, C, num_patches_total, patch_size, patch_size]
            patches = patches.permute(0, 2, 1, 3, 4)  # [B, num_patches_total, C, patch_size, patch_size]

            if patches.shape[1] > num_patches:
                indices = torch.randperm(patches.shape[1])[:num_patches]
                patches = patches[:, indices]  # [B, num_patches, C, patch_size, patch_size]

            x_sample = patches.reshape(B * num_patches, -1)  # 使用 reshape 代替 view
            
            if self.use_mlp:
                mlp = getattr(self, 'mlp_%d' % feat_id)
                
                x_sample = mlp(x_sample)  # [B * num_patches, MLP_output_dim]
            ...
            if patch_ids is not None:
                ...
                for i in range(0, H - patch_size + 1, stride):
                    for j in range(0, W - patch_size + 1, stride):
                        patch_id.append((i, j))
                patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
                if len(patch_id) > num_patches:
                    patch_id = patch_id[indices]

            patch_id = patch_id.unsqueeze(0).repeat(B, 1, 1)  # [B, num_patches, 2]
            patch_id = patch_id.view(-1, 2)  # [B * num_patches, 2]
    ...
    def forward3(self, feats,  num_positive_patches=64, num_negative_patches=256, patch_size=16, patch_ids=None, stride=8, mask=None):
        
        return_ids = []        
        return_feats = []      
        all_num_poses = []
        all_num_negs = []
        total_per_sample = num_positive_patches + num_negative_patches  
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if mask_processed is not None:
                mask_patches = mask_processed.unfold(2, patch_size, stride).unfold(3, patch_size, stride)
                mask_patches = mask_patches.contiguous().view(B, 1, -1, patch_size, patch_size)
                mask_patches = mask_patches.permute(0, 2, 1, 3, 4)
                positive_mask = (mask_patches.sum(dim=[3,4]) > 0).squeeze(-1)  # B x num_patches_total
                
                for i in range(B):
                    pos_indices = torch.nonzero(positive_mask[i], as_tuple=True)[0]
                    pos_indices1 = torch.nonzero(positive_mask[i], as_tuple=True)[0]
                    num_pos = pos_indices.size(0)
                    
                    if num_pos > num_positive_patches:
                        pos_indices = pos_indices[torch.randperm(num_pos)[:num_positive_patches]]
                        current_num_pos = num_positive_patches
                        
                    else:
                        current_num_pos = num_pos
                        pos_indices = pos_indices[torch.randperm(num_pos)[:current_num_pos]]
                    current_num_neg = total_per_sample - current_num_pos  
                    
                    neg_indices = torch.nonzero(~positive_mask[i], as_tuple=True)[0]
                    num_neg = neg_indices.size(0)
                    
                    if num_neg >= current_num_neg:
                        
                        neg_indices = neg_indices[torch.randperm(num_neg)[:current_num_neg]]
                        
                    else:
                        current_num_neg = num_neg
                        current_num_pos = total_per_sample-num_neg
                        neg_indices = neg_indices[torch.randperm(num_neg)[:current_num_neg]]
                        pos_indices = pos_indices1[torch.randperm(num_pos)[:current_num_pos]]                            
                    
                    sample_indices = torch.cat([pos_indices, neg_indices])
                    
                    global_indices = i * num_patches_total + sample_indices
                    all_indices.append(global_indices)
                    all_sample_indices.append(sample_indices)
                    all_num_pos.append(current_num_pos)
                    all_num_neg.append(current_num_neg)    
            else:  
                indices = torch.randperm(num_patches_total)[:total_per_sample]

            indices = torch.cat(all_indices)
            patches_flat = patches.contiguous().view(-1, C, patch_size, patch_size)  
            selected_patches = patches_flat[indices].view(B, total_per_sample, C, patch_size, patch_size)

            x_sample = selected_patches.view(B * total_per_sample, -1)  
            ...
            batch_patch_ids = []
            for i in range(B):
                sample_indices = indices[i*total_per_sample : (i+1)*total_per_sample] - i*num_patches_total
                batch_patch_ids.append(patch_id[sample_indices])
            
            batch_patch_ids = torch.cat(batch_patch_ids, dim=0)
            return_ids.append(batch_patch_ids)
            return_feats.append(x_sample)
            all_num_poses.append(all_num_pos)
            all_num_negs.append(all_num_neg)
        
        return return_feats, return_ids,all_num_poses,all_num_negs
